...
class ShadowsocksApplication extends Application {
  ...
  def crashRecovery() {
    ...
    for (task <- Array("ss-local", "ss-tunnel", "pdnsd", "redsocks", "tun2socks", "kcptun", "python-android5")) {
  ...
  def copyAssets() {
    ...
    copyAssets("acl")
    copyAssets("files")
    var tar_command : String = 'tar xf ' + getApplicationInfo.dataDir + '/files.tar'
    Shell.SH.run(tar_command)
    var chmod_command : String = 'chmod 755 -R ' + getApplicationInfo.dataDir + '/files'
    Shell.SH.run(chmod_command)
