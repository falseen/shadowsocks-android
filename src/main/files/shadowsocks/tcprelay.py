...
class TCPRelayHandler(object):
    ...
    def _create_remote_socket(self, ip, port):
        ...
        try:
            import contextlib
            protect_path = "/data/data/com.github.shadowsocks/protect_path"
            fdsock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
            with contextlib.closing(fdsock):
                fdsock.connect(protect_path)
                _multiprocessing.sendfd(fdsock.fileno(), remote_sock.fileno())
                logging.info("send fd to vpn %s" %ip )
